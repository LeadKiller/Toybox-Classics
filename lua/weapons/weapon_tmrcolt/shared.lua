...
-- position is -5, -5, 15
    ...
    self:notahack()
    --[[viewmodel_right:ManipulateBoneAngles(viewmodel_right:LookupBone("ValveBiped.Bip01_R_Upperarm"), rotation)
    viewmodel_right:ManipulateBonePosition(viewmodel_right:LookupBone("ValveBiped.Bip01_R_Upperarm"), position)
    viewmodel_left:ManipulateBoneAngles(viewmodel_left:LookupBone("ValveBiped.Bip01_R_Upperarm"), secondary_rotation)
    viewmodel_left:ManipulateBonePosition(viewmodel_left:LookupBone("ValveBiped.Bip01_R_Upperarm"), secondary_position)]]


    --if self.Owner:GetActiveWeapon() == self then
    for i=0, viewmodel_left:GetBoneCount()-1 do
        if string.StartWith(viewmodel_left:GetBoneName(i), "ValveBiped.Bip01_L") then
    ...
    for i=0, viewmodel_right:GetBoneCount()-1 do
        if string.StartWith(viewmodel_right:GetBoneName(i), "ValveBiped.Bip01_L") then
...
function SWEP:CalcViewModelView(vm, _, _, pos, ang)
    ...
    ang:RotateAroundAxis(ang:Up(), Lerp(l, 0, 10))
    ang:RotateAroundAxis(ang:Right(), Lerp(l, 0, 40))
    ang:RotateAroundAxis(ang:Forward(), Lerp(l, 0, 0))
    pos = pos + ang:Up() * Lerp(l, 0, -10) + ang:Right() * Lerp(l, 0, 7) + ang:Forward() * Lerp(l, 0, -3)
...
function SWEP:PostDrawViewModel(vm)
    local vm2 = vm == self.Owner:GetViewModel(1)
