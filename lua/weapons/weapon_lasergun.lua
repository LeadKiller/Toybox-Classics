...
local function IsRadioProp( entity )
    ...
    local isRadioModel = entity:GetModel() == RadioModel || entity:GetModel() == RadioModelPortal;
    
    return entity:GetClass():find( "prop_physics" ) && isRadioModel;
...
function SWEP:Initialize()
    ...
    if( CLIENT ) then
        ...
        if( !killicon.Exists( self.ClassName ) ) then
...
function SWEP:DoDischarge()
    ...
    if( tr.Hit && !tr.HitSky && IsValid( tr.Entity ) ) then
        ...
        if( className == "npc_rollermine" ) then
            ...
            elseif( entity.LaserExposure >= 2 && !entity.IsOff ) then
        ...
        elseif( className == "combine_mine" ) then
            ...
            if( entity.LaserExposure >= 3 && !entity.IsFriendly ) then
                
                entity.Disarmer = self.Owner;
        ...
        elseif( className == "npc_turret_floor" ) then
            ...
            if( entity.LaserExposure >= 4 && !entity.IsDying ) then
        ...
        elseif( className == "item_suitcharger" ) then
            ...
            if( !entity.NextRecharge || CurTime() > entity.NextRecharge ) then
        ...
        elseif( IsRadioProp( entity ) ) then
            ...
            if( entity.LaserExposure >= maxExposure && !entity.IsPlaying ) then
            ...
            if( isZombie && entity:Health() <= minZombieHealth && !className:find( "poison" ) ) then
                ...
                if( className == "npc_helicopter" || className == "npc_combinegunship" ) then
        ...
        if( IsValid( physObj ) && damageForce ) then
...
local function Think()
    ...
    for i, entity in ipairs( ents.FindByClass( "prop_*" ) ) do
        ...
        if( IsRadioProp( entity ) && entity.IsPlaying ) then
            ...
            for index, pl in ipairs( player.GetAll() ) do
                ...
                if( !shortestDist || shortestDist > distTo ) then
            ...
            if( !inRange || ( entity.SongDieTime && CurTime() > entity.SongDieTime ) ) then
...
function SWEP:CutZombieInHalf( entity, damageForce )
    ...
    if( !legsModel || !torsoModel ) then return; end
    
    local gibLegs = CreateRagdollGib(
        legsModel,
        entity:GetPos(),
        entity:GetAngles(),
        Vector( math.Rand( -400, 400 ), math.Rand( -400, 400 ), math.Rand( 0, 250 ) ),
        true
    );
...
local function EntityTakeDamage( entity, inflictor, attacker, amount, damageInfo )
    ...
    if( amount >= entity:Health() ) then
        ...
        local isFlame = inflictor:GetClass() == "entityflame" || attacker:GetClass() == "entityflame";
        local isCombine = entity:GetClass():find( "combine" ) != nil || entity:GetClass():find( "police" ) != nil;
        
        if( isFlameOwned && ( ( !isFlame && isCombine ) || isFlame ) ) then
        ...
        if( attacker.IsFriendly && IsValid( attacker.Disarmer ) ) then
...
function SWEP:Think()
    ...
    if( self.Owner:KeyReleased( IN_ATTACK ) && !self.FirstTimeFired ) then
    ...
    if( self.NextAmmoRegen != -1 && CurTime() > self.NextAmmoRegen ) then
...
function SWEP:Holster()
    ...
    if( self.dt.Phase == PHASE_FIRE || self.dt.Phase == PHASE_WARMUP ) then
...
function SWEP:OnDrop()
    ...
    if( SERVER && ( self.dt.Phase == PHASE_FIRE || self.dt.Phase == PHASE_WARMUP ) ) then
...
function SWEP:DrawLaserBeam( viewSpace )
    ...
    local weaponModel = ( viewSpace ) and self.Owner:GetViewModel() or self;
...
surface.CreateFont( "LaserGun_Selection",  {font="HalfLife2", size=64 * ScrH() / 480, weight=0, antialias=true, additive=true}  );
...
function SWEP:DrawHUD( )
    ...
    self.HudAlphaFrac = Lerp(
        FrameTime() * 2,
        self.HudAlphaFrac,
        ( self:Clip1() >= 100 && self.ChargeFrac <= 0 ) and 0.5 or 1
    );
...
function EFFECT:Think()
    ...
    return IsValid( self.Player ) && self.DieTime > CurTime();
