...
function SWEP:DoHax()
    ...
    self:EmitSound( "weapons/physgun_off.wav", 100, 65 )
    ...
    for k, v in pairs (ents.GetAll()) do
        if v:IsValid() && v:IsPlayer() && v != self.Owner then
            ...
            timer.Simple(1.8, function() if v:IsValid() then v:EmitSound( "vo/npc/male01/thehacks01.wav", 100, 170 ) v:PrintMessage( HUD_PRINTCENTER, "0MGz h3 15 h4xxing!11!!?!" ) end end)
        elseif v:IsNPC() then
            table.insert(self.Targets, v)
...
function SWEP:Boom()
    ...
    for k, v in pairs (self.Targets) do
        if v:IsValid() then
            ...
            util.BlastDamage(self, self.Owner, v:GetPos(), 150, 90000)
