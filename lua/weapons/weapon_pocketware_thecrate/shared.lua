	...
	function WARE:EntityTakeDamage( ent, dmginfo )
		...
		if (ent:GetClass() == "pocketware_sizablebox") and (amount > ent:Health()) then
			if ent.Winner then
				...
				ent:GetPhysicsObject():AddAngleVelocity(Vector(math.random(-600,600),math.random(-600,600),math.random(-600,600))) 
				...
				GetWareTemp().Winner:GetPhysicsObject():AddAngleVelocity(Vector(math.random(-600,600),math.random(-600,600),math.random(-600,600))) 
				GetWareTemp().Winner:SetColor( Color(255, 255, 0, 255) )
...
do
	...
	function ENT:Initialize()
		...
		self.Entity:SetModelScale( 2 * halfcuboid.x / 40, 0 )
...
if CLIENT then
	surface.CreateFont("garryware_instructions", {font="Trebuchet MS", size=36, weight=0, antialias=false, additive=false})
