...
if CLIENT then
	surface.CreateFont("TextFont", {font="Segoe Print", size=20, weight=550} )
end
        ...
        if self.Weapon:Clip1() <20 then clipcol = Color(255,0,0,255) elseif self.Weapon:Clip1() <60 then clipcol = Color(255,255,0,255) elseif self.Weapon:Clip1() >59 then clipcol = Color(0,255,0,255) end
        if self.Weapon:Clip2() == 0 then clip2col = Color(255,0,0,255) elseif self.Weapon:Clip2() == 1 then clip2col = Color(0,255,0,255) end
...
function SWEP:CustomAmmoDisplay()
   ...
   phys:ApplyForceCenter (self.Owner:GetAimVector() * 1000)
    ...
    //effects
       ...
       diagonall2:SetOwner(self.Owner)
       ...
       diagonald2:SetOwner(self.Owner)
       ...
       diagonalu2:SetOwner(self.Owner)
    ...
    local rad = ents.Create( "point_hurt" )
       ...
       self:Remove()
        ...
        if self.Owner:KeyDown(IN_ATTACK) || self.Owner:KeyDown(IN_ATTACK2) then return 
        else
        
        local NextThink = 0 
            
        if( CurTime() >= NextThink ) && (self:Clip1() < 100 ) && (self:Clip1() > 1 )then 
        self:SetClip1( self:Clip1() + 1 ) 
        NextThink = CurTime() + 0.1    
        end
  
        if( CurTime() >= NextThink ) && (self:Clip2() < 1 ) && (self:Clip2() > -1 )then 
