...
function ENT:Move()
    ...
    while path:IsValid() and IsValid(enemy) do
        ...
        if path:GetAllSegments() and path:GetAllSegments()[1] and path:GetAllSegments()[2] and path:GetAllSegments()[2].pos.z > self:GetPos().z and path:GetAllSegments()[2].pos.z < self:GetPos().z + 24 and !(util.TraceHull({startpos = self:GetPos(), endpos = self:GetPos() + self:GetForward() * 50 + self:GetUp() * 24, mins = Vector(-15, -15, 0), maxs = Vector(15, 15, 72)}).Entity:IsPlayer() or self:GetPos():DistToSqr(enemy:GetPos()) < 2400) then
            self:SetPos(self:GetPos() + path:GetAllSegments()[1].forward)
        end
...
function ENT:Eyes()
    if !IsValid(self:GetNWEntity("enemy")) then return end
    local enemy = self:GetNWEntity("enemy")
    if !enemy:Alive() then return true end
...
function ENT:OnRemove()
    ...
    if !IsValid(self) then return end -- doesn't exist!
    if !IsValid(flemming) then return end
    ...
    timer.Simple(10, function()
        if isentity(flemming) and IsValid(flemming) then -- you won fair and square, guess i'm gone!
